...
class Router:
    ...
    def dispatch(self, handle, paramstring):
        ...
        elif mode == 'configureupdateurls':  # Main menu → Update URL
            from resources.libs import restore
            restore.configure_update_urls()
            xbmc.executebuiltin('Container.Refresh()')
        elif mode == 'buildupdate':  # Update → Run (hardcoded URLs only)
            from resources.libs import restore
        ...
        # BACKUP / RESTORE
            ...
            from resources.libs import restore
            restore.restore(action, external=name == 'external', from_url=name == 'url')
