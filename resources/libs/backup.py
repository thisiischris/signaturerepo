...
# Subfolder names under addon_data often used for tokens, OAuth, or saved accounts.
_SENSITIVE_ADDON_SUBDIRS = frozenset({
    'tokens', 'oauth', 'oauth2', 'auth', 'accounts', 'credentials', 'secrets',
    'login', 'logins', 'private', 'secure', 'api', 'authorization', 'session',
    'keyring', 'passwords', 'passwd', 'signin',
})


def _sensitive_addon_data_exclude_roots():
    """addon_data paths OpenWizard knows store Trakt/Debrid/logins/API keys (whole add-on folder)."""
    roots = [CONFIG.TRAKTFOLD, CONFIG.DEBRIDFOLD, CONFIG.LOGINFOLD]
    try:
        from resources.libs import traktit, debridit, loginit
        for reg in (traktit.TRAKTID, debridit.DEBRIDID, loginit.LOGINID):
            for meta in reg.values():
                settings_xml = meta.get('settings')
                if not settings_xml:
                    continue
                roots.append(os.path.dirname(settings_xml))
    except Exception:
        pass
    seen = set()
    out = []
...
class Backup:
    ...
    def backup_build(self, name=""):
        if self.dialog.yesno(CONFIG.ADDONTITLE,
            ...
            strip_sensitive_addon_data = self.dialog.yesno(
                CONFIG.ADDONTITLE,
                "[COLOR {0}]Exclude sensitive addon_data from this backup?[/COLOR]".format(CONFIG.COLOR2) + '\n'
                + "If yes: skips this wizard\'s saved Trakt/Debrid/Login folders, "
                + "addon folders OpenWizard tracks for those services, and common subfolders such as "
                + "[COLOR {0}]tokens, accounts, oauth, auth, api, credentials[/COLOR] under addon_data. "
                + "Other add-on settings (skins, etc.) are still included.".format(CONFIG.COLOR1),
                nolabel='[B][COLOR springgreen]Keep all addon_data[/COLOR][/B]',
                yeslabel='[B][COLOR red]Exclude sensitive[/COLOR][/B]')
            if strip_sensitive_addon_data:
                exclude_dirs.extend(_sensitive_addon_data_exclude_roots())

            excluded_norm = {os.path.normcase(os.path.normpath(p)) for p in exclude_dirs}
            _addon_data_nc = os.path.normcase(os.path.normpath(CONFIG.ADDON_DATA))

            def _prune_walk_dirs(base, dirs):
                base_nc = os.path.normcase(os.path.normpath(base))
                inside_ad = base_nc == _addon_data_nc or base_nc.startswith(_addon_data_nc + os.sep)
                keep = []
                for d in dirs:
                    cand = os.path.normcase(os.path.normpath(os.path.join(base, d)))
                    if cand in excluded_norm:
                        continue
                    if strip_sensitive_addon_data and inside_ad and d.lower() in _SENSITIVE_ADDON_SUBDIRS:
                        continue
                    keep.append(d)
                dirs[:] = keep
