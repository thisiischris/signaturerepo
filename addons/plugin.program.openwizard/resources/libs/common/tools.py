...
def open_url(url, stream=False, check=False, cred=None, count=0):
    ...
    if not valid:
        ...
        # Connect timeout; read timeout between chunks (large zips need a high read limit).
        req_timeout = (30.0, 3600.0) if stream else 10.0
        response = requests.get(url, headers=user_agent, timeout=req_timeout, stream=stream, auth=cred)
